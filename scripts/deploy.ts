...
async function main() {
  ...
  const speakerManager = await upgradesApi.deployProxy(SpeakerManager, [deployer.address], {
  ...
  const seminarManager = await upgradesApi.deployProxy(SeminarManager, [deployer.address], {
    initializer: "initialize",
  });
  ...
  const seminarRandomizer = await upgradesApi.deployProxy(SeminarRandomizer, [deployer.address], {
    initializer: "initialize",
  });
  ...
  console.log("Wiring source contracts into SeminarRandomizer...");
  await seminarRandomizer.setSourceContracts(speakerManagerAddress, seminarManagerAddress);
  console.log("SeminarRandomizer sources configured.");
